        ...
        var am = {
            ...
            global: {
                ...
                setTimeOut: function (para) {
                    ...
                    function setinput() {
                        $(document.body).undelegate('input', 'input.page');
                        $(document.body).undelegate('textarea', 'input.page');

                        $(document.body).delegate('input', 'input.page', function () {
                            //console.log("input page");
                            _para.timeOut = para.timeOut;
                        });
                        $(document.body).delegate('textarea', 'input.page', function () {
                            _para.timeOut = para.timeOut;
                        });
                    }
                ...
                */
                    ...
                    $(window).on('input.SpecialSymbol', function (event) {
                        ...
                        if (!ingoreGlobalFilter) {
                            ...
                            setTimeout(function () {
                                ...
                                if (event.target.value.indexOf('ü') >= 0)
                                    return;
                ...
                swalllowCardAndSendWq: function (para) {
                    ...
                    if (window.AppHost == null || cm.client.clientType == null || (cm.client.clientType != "C" && cm.client.clientType != "CM"))
                        dfd.resolve();
                    else {
                ...
                //记录屏幕四角点击注销
                RecorderClick: function (par) {
                    if (window.AppHost == null)
                        return;
                    var flgStr = 'a,b,c,d';

                    if (am.screenClickCount().length > 0) {
                        var lastValue = am.screenClickCount()[am.screenClickCount().length - 1];
                        if (lastValue == par)
                            return;
                        else
                            am.screenClickCount.push(par);

                        am.global.log(am.screenClickCount().toString());

                        if (am.screenClickCount().toString().indexOf(flgStr) >= 0) {
                            am.global.log("abcb");
                            var device = new jsbridge.Manage();
                            device.logoff().then(function () {
                            });
                        }
                    }
                    else {
                        if (par == 'a') {
                            setTimeout(function () { am.screenClickCount.removeAll(); }, 60000);
                            am.screenClickCount.push(par);
                            am.global.log(am.screenClickCount().toString());
                        }
                    }
